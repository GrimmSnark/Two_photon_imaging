...
if isempty(templateImg)
    templateImg = prepData([recordingDir fileList(templateRecordingNum).name], 1, 1, 0, experimentType, []);
end

folderNum = length(recordingFolders);
...
for i =folderNum
    templateImgArray(:,:,i) = prepData([recordingDir fileList(i).name], 1, 1, 0, experimentType, []); % preps the rest of the imaging files
end

recordingTemplate = mean(templateImgArray,3); % makes average of all the movie averages for ROI selection
